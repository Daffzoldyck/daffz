...
const isAdmin = user?.email === "dazht156@gmail.com";
      ...
      <div className="h-72 overflow-y-auto border border-gray-700 p-3 rounded-lg bg-zinc-800 mb-4 space-y-3">
        {messages.map((msg) => (
          ...
          >
            ...
            <div
  className={`p-3 rounded-lg max-w-[75%] relative overflow-visible ${
    msg.uid === user?.uid
...
{(msg.uid === user?.uid || isAdmin) && (
  ...
  <button
    ...
    className="absolute top-1 right-1 z-50 text-white bg-red-600 rounded-full px-2 py-1 text-xs hover:bg-red-700"
  >
    🗑️
  </button>
)}
